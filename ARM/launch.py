...
# dist.init_process_group(backend='nccl',init_method='tcp://localhost:%d' % port,rank=0, world_size=1) #xiong
...
def run_seed(cfg: DictConfig, env, cams, train_device, env_device, seed) -> None:
    ...
    env_runner = EnvRunner(
        ...
        training_iterations=cfg.framework.training_iterations, #episodes=99999 xiong 有问题！
        episode_length=cfg.rlbench.episode_length,
        stat_accumulator=stat_accum,
        weightsdir=weightsdir,
        env_device=env_device,
        rollout_generator=rg,
        rollout_episodes=20, #xiong
        eval_episodes=5, #xiong
        eval_from_eps_number=100, #xiong
        )
...
def main(cfg: DictConfig) -> None:
    ...
    # print('task.TASKS_PATH:', task.TASKS_PATH)
